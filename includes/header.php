...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col">
    ...
    <header class="bg-brand-700 text-white sticky top-0 z-30 shadow">
      <div class="max-w-7xl mx-auto px-4 flex items-center justify-between h-14">
        <div class="flex items-center gap-6">
          <a href="<?php echo BASE_URL ?>/<?php echo $user['role'] ?>/dashboard.php" class="font-bold text-lg tracking-tight">
            📚 QL Nhóm & Đề tài
          </a>
        ...
        <div class="flex items-center gap-3 text-sm">
          <span class="hidden sm:inline text-brand-100"><?php echo $user['ho_ten'] ?> · <span class="uppercase text-xs bg-brand-800 px-2 py-0.5 rounded"><?php echo $user['role'] ?></span></span>
          <a href="<?php echo BASE_URL ?>/logout.php" class="bg-brand-800 hover:bg-brand-900 px-3 py-1.5 rounded transition">Đăng xuất</a>
        </div>
      </div>
    </header>
  <?php endif; ?>
  ...
  <main class="flex-1 max-w-7xl w-full mx-auto px-4 py-6">
    ...
    if ($flash): ?>
      ...
      <?php echo $flash['type'] === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200' ?>">
