...
", [$id]);
if (!$nhom) {
  set_flash('error', 'Không tìm thấy nhóm.');
...
if (isset($_POST['moi'])) {
  csrf_check();
  if (!$isLeader) {
    set_flash('error', 'Chỉ trưởng nhóm mới được mời thành viên.');
    redirect('/sinhvien/nhom.php?id=' . $id);
  }
  if (is_qua_han($nhom['han_dang_ky_nhom'])) {
    set_flash('error', 'Đã hết hạn đăng ký nhóm, không thể mời thêm.');
    redirect('/sinhvien/nhom.php?id=' . $id);
  }
