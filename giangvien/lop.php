...
if (isset($_POST['toggle_dot'])) {
  ...
  $chk = db_query_one('SELECT id, trang_thai FROM dot_dangky WHERE id=? AND lop_id=?', [$dot_id, $id]);
  if ($chk) {
    $status = $chk['trang_thai'];
    $status = $status === 'mo' ? 'dong' : 'mo';
    db_exec("UPDATE dot_dangky SET trang_thai = ? WHERE id=?", [$status, $dot_id]);
  }
  redirect('/giangvien/lop.php?id=' . $id);
}
...
function chiaNhom(array $svIds, int $min, int $max): array
{
  $n = count($svIds);

  for ($groups = ceil($n / $max); $groups <= floor($n / $min); $groups++) {

    $base = intdiv($n, $groups);
    $extra = $n % $groups;

    if ($base < $min || $base + ($extra ? 1 : 0) > $max) {
      continue;
    }
    ...
    for ($i = 0; $i < $groups; $i++) {
      ...
      $result[] = array_slice($svIds, $index, $size);
      $index += $size;
    }
...
if (isset($_POST['random_nhom'])) {
  ...
  $chunks = chiaNhom(
    $svIds,
    (int)$lop['si_so_nhom_toi_thieu'],
    (int)$lop['si_so_nhom_toi_da']
  );

  if (empty($chunks)) {
    set_flash('error', 'Không thể chia nhóm với điều kiện hiện tại.');
    redirect('/giangvien/lop.php?id=' . $id);
  }
  $soNhomTao = 0;
...
if (isset($_POST['chia_nhom'])) {
  ...
  if (empty($svIds)) {
    set_flash('error', 'Không có sinh viên nào để chia nhóm.');
    redirect('/giangvien/lop.php?id=' . $id);
  }
    ...
    <div class="grid md:grid-cols-2 gap-4 mb-6">
      <div class="bg-white border border-slate-200 rounded-xl p-4 text-sm">
        ...
        <div class="flex gap-2 mt-3">
          ...
          <form method="post" class="flex-1" onsubmit="return confirm('Random chia nhóm cho <?php echo $svChuaCoNhomCount ?> sinh viên chưa có nhóm? (sẽ giải tán các nhóm chưa đủ sĩ số)');">
            <?php echo csrf_field() ?>
            <button class="w-full text-xs bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-1.5 rounded" name="chia_nhom">
              🎲 Chia nhóm</button>
          </form>
        </div>
      </div>
